    ...
    <footer className="relative overflow-hidden bg-ink text-white">
      ...
      <div className="container-wide border-b border-white/10 py-16 sm:py-20">
        <div className="flex flex-col items-start justify-between gap-8 lg:flex-row lg:items-center">
          ...
          <Link
            to="/contact"
            className="group inline-flex items-center gap-3 rounded-full bg-accent px-7 py-4 text-sm font-semibold text-ink transition-transform hover:scale-105"
          >
            Get A Quote
            <span className="grid h-7 w-7 place-items-center rounded-full bg-ink text-accent transition-transform group-hover:rotate-45">
          ...
          <div className="mt-6 flex gap-2">
            {COMPANY.socials.map((s) => (
              <a
                ...
                href={s.href}
                aria-label={s.label}
                className="grid h-10 w-10 place-items-center rounded-full border border-white/15 text-xs font-semibold text-white/70 transition-colors hover:border-accent hover:bg-accent hover:text-ink"
              >
                {s.label[0]}
              </a>
            ))}
          ...
          <form onSubmit={submit} className="mt-4">
            <div className="flex items-center gap-2 rounded-full border border-white/15 p-1.5 focus-within:border-accent">
              ...
              <button
                ...
                className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-accent text-ink"
              >
                <Icon name="arrow" size={15} />
              </button>
            </div>
