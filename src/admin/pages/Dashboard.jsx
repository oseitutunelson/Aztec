...
const PIE_COLORS = ['#FF6B2C', '#0E0E0E', '#6B6B6B', '#FF8551', '#E0561F', '#A3A3A3', '#404040', '#FFB088']
        ...
        <ChartCard title="Projects by Category">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={charts?.projectsByCategory || []}>
              ...
              <Tooltip contentStyle={tooltipStyle} cursor={{ fill: 'rgba(255,107,44,0.08)' }} />
              <Bar dataKey="value" fill="#FF6B2C" radius={[6, 6, 0, 0]} maxBarSize={48} />
            </BarChart>
          </ResponsiveContainer>
        </ChartCard>
        ...
        <ChartCard title="Monthly Website Traffic" note="Sample data">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={charts?.traffic || []}>
              ...
              <Tooltip contentStyle={tooltipStyle} />
              <Area type="monotone" dataKey="visitors" stroke="#FF6B2C" strokeWidth={2} fill="url(#traffic)" />
            </AreaChart>
          </ResponsiveContainer>
        </ChartCard>
        ...
        <ChartCard title="Contact Requests" note={(charts?.contactRequests?.length ?? 0) === 0 ? 'No data yet' : undefined}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={charts?.contactRequests || []}>
              ...
              <YAxis tick={{ fill: axisColor, fontSize: 11 }} tickLine={false} axisLine={false} allowDecimals={false} />
              <Tooltip contentStyle={tooltipStyle} />
              <Line type="monotone" dataKey="count" stroke="#0E0E0E" strokeWidth={2} dot={{ r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        </ChartCard>
      </div>
